...
class DashboardController extends Controller
{
	...
	*/
	...
	{
		$addresses = Address::groupBy('city')
		->select('addresses.*', DB::raw('count(*) as total'))->get();
		$cities = Terms::where("type", 2)->pluck('title')->toArray();
		$cities = array_map('strtolower', $cities);
		return view('plugin::admin.address_request')->with("addresses", $addresses)->with("cities", $cities);
	}
}
