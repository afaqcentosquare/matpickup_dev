...
class FrontendController extends Controller
{
	...
	{
		$allCities= Terms::where('type',2)->where('status',1)->with('preview')->get();
		return response()->json($allCities);
	}

	public function allCitiesStores($id)
	{
	...
	public function allStoreCategories($slug)
	{
        ...
        // return response()->json($store_id);
		 ...
		 return response()->json(["store_data" => $store_id , "categories" => $categories]);
	}

	public function categoryProducts($slug, $store_id)
	{
		$category_products = Category::where("slug", $slug)->where("user_id", $store_id)->first();
        
        //return response()->json($category_products);
        $products = $category_products->products()->paginate(12);

		return response()->json(["products" => $products]);
	}

	public function stripeCreatePayment(Request $request)
	{
		...
		try{
			\Stripe\Stripe::setApiKey('sk_test_51J3ii6G3T1oQY9SE9eMUFmcTvXygqFT3MRkNqM0UkuvB2TW5hZlvGKUPmWk2cdmjPJJMGWWmdspEoAmUARzHrSLx00fW4vI6yM');
