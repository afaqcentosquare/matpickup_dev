...
class OrderController extends Controller
{
	...
	public function store(Request $request)
	{
		...
		try{
	   	...
	   	$datacart = json_decode($request->datacart);
	   	foreach($datacart as $key => $value) {
	       	$ordermeta = new Ordermeta;
	          $ordermeta->order_id = $order->id;
	          $ordermeta->qty  = $value->product_quantity;
	          $ordermeta->total  = $value->product_price;
	          $ordermeta->term_id  = $value->product_id;
	          $ordermeta->save();
	      }
			return $order;

		//return response()->json(json_decode($request->datacart));
		}catch(Exception $ex){
