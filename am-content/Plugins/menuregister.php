	...
	if(Auth::User()->role->id == 1){
		...
		if (Amcoders\Plugin\Plugin::is_active('product')) {
			if (Auth()->user()->can('product.list')) {
				...
				$products["Product Checker"] = route('admin.check.product.availability');
			}
		...
		// if (Auth()->user()->can('address.requests')) {
			$data['admin_address_request']=array(
				'name' => "Address Requests",
				'active' => Request::is('admin/address/requests'),
				'icon' => 'fas fa-tachometer-alt',
				'url' => route('admin.address.requests')
			);
		// }
