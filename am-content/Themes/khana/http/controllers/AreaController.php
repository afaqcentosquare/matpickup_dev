...
class AreaController extends controller
{
	...
	public function index($slug)
	{
		...
		//$infooo=Terms::where('status',1)->where('type',2)->with('excerpt')->find(1);
	...
	//return response()->json($allCities);
	  ...
	  return view('theme::frontend.area_stores')->with("allCities", $allCities)->with("allCitiesStores", $allCitiesStores);
	}

	public function searchByCity(Request $request)
	{
		if (is_numeric($request->postal_code)){
			$locations = Location::where('locations.role_id',3)
			->where('zip_code', $request->postal_code)
			->wherehas('users')
			->with('users')
			->get();
			...
			$cities = Terms::where('type',2)->where("title", $request->postal_code)->pluck("id");
			
			$locations = Location::where('locations.role_id',3)
			->whereIn("term_id",$cities)
			->wherehas('users')
			->with('users')
			->get();
		}
		...
		if($locations->isEmpty()){
			if (Auth::check()) {
				...
				return view('theme::frontend.auth');
				//return view('theme::login.index');
			}
		}else{
			return view("theme::frontend.stores")->with("locations",$locations);
		}
